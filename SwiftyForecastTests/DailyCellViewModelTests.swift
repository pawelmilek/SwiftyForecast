...
class DailyCellViewModelTests: XCTestCase {
  ...
  private var sut: DailyCellViewModel!
  ...
  override func setUp() {
    ...
    sut = DefaultDailyCellViewModel(dailyData: currentDayData, notationController: notationController)
  }
  ...
  func testConditionIcon() {
    ...
    let expected = ConditionFontIcon.make(icon: dailyIcon, font: Style.DailyForecastCell.conditionIconSize)?.attributedIcon
