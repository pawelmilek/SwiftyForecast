...
class WeatherDatastore {
  private var darkURL = ""
  private weak var city: City? = nil
  
  
  // MARK: - Retrieve Current Weather
  func retrieveCurrentWeather(at coordinate: Coordinate, block: @escaping (_ condition: Weather) -> Void) {
    ...
    Alamofire.request(self.darkURL).responseJSON { response in
      if(response.result.value == nil) {
        ...
        Geocoder.findCity(at: coordinate) { cityLocation in
  ...
  func retrieveHourlyWeather(at coordinate: Coordinate, forecast: Int, block: @escaping (_ conditions: [Weather]) -> Void) {
  ...
  func retrieveDailyForecast(at coordinate: Coordinate, forecast: Int, block: @escaping (_ conditions: [Weather]) -> Void) {
...
private extension WeatherDatastore {
  ...
  func createDayForecast(from json: JSON) -> Weather {
    ...
    let weather = Weather(city: nil, timeStamp: Int(truncating: timestamp), icon: icon, description: summary, currentTemp: nil, minTemp: minTempFahr, maxTemp: maxTempFahr, moonPhaseLunation: Float(moonPhase))
